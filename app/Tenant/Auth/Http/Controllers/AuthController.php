...
{
    private const TENANT_LOGIN_USER_TYPES = [
    ...
    public function login(LoginRequest $request): JsonResponse
    {
        ...
        if (! $this->canUseTenantLogin($user)) {
            $this->logs->permission('tenant.login.blocked', [
                'tenant_id'    => $user->tenant_id,
                'entity_type'  => 'tenant_session',
                'entity_id'    => $user->id,
                'entity_label' => $user->email,
                'metadata'     => ['reason' => 'tenant_login_role_not_allowed'],
            ], $user, $request);
    ...
    private function canUseTenantLogin(User $user): bool
    {
        $userType = $user->user_type?->value ?? $user->user_type;

        return in_array($userType, self::TENANT_LOGIN_USER_TYPES, true);
    }
    ...
    {
        try {
            return (string) app(Tenant::class)->getTenantKey();
        } catch (\Throwable) {
